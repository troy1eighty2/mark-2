...
const addTooltipForKeychains = function ($container, imageUrl, hashName, name, price, pattern) {
  if ($container.next('.tooltip').length === 0) {
    $container.after(`<div class="tooltip"></div>`);
  }

  if ($container.parent().find(`.tooltip__row`).length === 0) {
    const rowEl = `<div class="tooltip__row">
        <div class="tooltip__left-block">
          <div class="tooltip__image"><img src="${imageUrl}" title="${hashName}"></div>
          <div class="tooltip__name">${name}</div>
        </div>
        <div class="tooltip__right-block tooltip__right-block_keychain">
          <span class="tooltip__price">${price}</span>
          <span class="tooltip__percent">Paint seed: ${pattern}</span>
        </div>
      </div>`;
    $container.parent().find('.tooltip').append(rowEl);
  }
};
...
const processFloatQueue = function () {
  ...
  Promise.all(
    floatQueue.map(({ listingId, inspectLink, isFloat = true, stickersCount, hasKeychain }, index) => {
      setTimeout(
        () => {
          ...
          GetFloat(listingId, inspectLink)
            .then((data) => {
              if (
                data &&
                data.iteminfo &&
                (data.iteminfo.stickers.length !== stickersCount || !!data.iteminfo.keychains?.length !== !!hasKeychain)
              ) {
